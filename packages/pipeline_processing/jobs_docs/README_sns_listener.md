...
The SNS HTTP listener (`packages/pipeline_processing/jobs/sns_listener.py`) implements a small in-process HTTP server that accepts AWS SNS HTTP POST requests and forwards the inner S3 event message to the data quality job.
...
The image must be built first (`./packages/pipeline_processing/build.sh`).
...
poetry run pipeline-processing run sns_listener

# Verify container is running and attached to the network
docker ps --filter name=pipeline-processing-sns_listener
```
...
docker run --rm --network "$NETWORK" -e AWS_ACCESS_KEY_ID=test -e AWS_SECRET_ACCESS_KEY=test \
  ...
  sns subscribe --topic-arn "$TOPIC_ARN" --protocol http --notification-endpoint "http://pipeline-processing-listener:8080/"
...
EVENT=$(python -c 'import json,sys; print(json.dumps(json.load(open("packages/pipeline_processing/event.json"))))')
...
docker logs -f pipeline-processing-listener
...
export DJANGO_SETTINGS_MODULE=pipeline_processing.settings
poetry run python -m pipeline_processing.cli.main run sns_listener
```
