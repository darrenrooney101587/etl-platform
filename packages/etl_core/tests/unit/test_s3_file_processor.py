"""
Tests for the S3FileProcessor class (migrated to etl_core).

These tests were adapted to point at `etl_core.processors.s3_file_processor` and
use the unittest style used across the repo.
"""
from etl_core.config.config import S3Config
from etl_core.processors.s3_file_processor import S3FileProcessor
from etl_core.tests.unit.conftest import setup_mocks
...
class TestS3FileProcessor(unittest.TestCase):
    ...
    @patch("etl_core.processors.s3_file_processor.ThreadPoolExecutor")
    def test_process_files_success(self, mock_executor_class: Mock) -> None:
        ...
        with patch("etl_core.processors.s3_file_processor.as_completed") as mock_as_completed:
    ...
    @patch("etl_core.processors.s3_file_processor.time.sleep")
